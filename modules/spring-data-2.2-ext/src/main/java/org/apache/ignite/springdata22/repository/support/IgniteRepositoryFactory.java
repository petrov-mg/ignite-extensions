...
public class IgniteRepositoryFactory extends RepositoryFactorySupport {
    ...
    static final String TEXT_QRY_UNSUPPORTED_ERR = "Invalid Spring Data query configuration for method" +
        " %s#%s. Text queries are not suppported when a client node is used to access the Ignite cluster." +
        " Check \"igniteInstance\" property of repository configuration.";
    ...
    private IgniteCacheProxy<?, ?> getRepositoryCache(Class<?> repoIf) {
        ...
        IgniteCacheProxy<?, ?> c = config.autoCreateCache() ?
    ...
    @Override protected Optional<QueryLookupStrategy> getQueryLookupStrategy(final QueryLookupStrategy.Key key,
        ...
        return Optional.of((mtd, metadata, factory, namedQueries) -> {
            ...
            final boolean isClient = repoToIgnite.get(metadata.getRepositoryInterface()) instanceof IgniteClientProxy;
            ...
            if (annotation != null && (StringUtils.hasText(annotation.value()) || annotation.textQuery() || annotation
                ...
                if (isTextQry && isClient)
                    throw new IgniteException(format(TEXT_QRY_UNSUPPORTED_ERR, mtd.getDeclaringClass(), mtd.getName()));
            ...
            IgniteQuery sqlQry = IgniteQueryGenerator.generateSql(mtd, metadata);

            if (sqlQry.isTextQuery() && isClient)
                throw new IgniteException(format(TEXT_QRY_UNSUPPORTED_ERR, mtd.getDeclaringClass(), mtd.getName()));

            return new IgniteRepositoryQuery(metadata, sqlQry, mtd,
