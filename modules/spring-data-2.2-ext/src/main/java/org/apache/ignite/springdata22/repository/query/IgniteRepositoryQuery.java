...
public class IgniteRepositoryQuery implements RepositoryQuery {
    ...
    /** Fields descriptors of domain entitiy fields. */
    ...
    public IgniteRepositoryQuery(
        ...
        if (dynamicQueryConfigurationIndex == -1 && this.staticQuery == null) {
            throw new IllegalStateException(
                "When passing dynamicQuery = true via org.apache.ignite.springdata22.config.Query "
    ...
    private ReturnStrategy getReturnStgy(IgniteQuery qry) {
        ...
        throw new IllegalStateException("Unable to obtain a valid return strategy. When passing dynamicQuery = true "
            + "via org.apache.ignite.springdata22.config.Query annotation, "
    ...
    /**
     * Maps SQL result row values to  to corresponing result column name.
     *
    ...
    private static Map<String, Object> rowToMap(final List<?> row, FieldsQueryCursor<?> cursor) {
        ...
        final TreeMap<String, Object> res = new TreeMap<>(String.CASE_INSENSITIVE_ORDER);

        for (int i = 0; i < row.size(); i++) {
            // don't want key or val columns
            final String fieldName = cursor.getFieldName(i).toLowerCase();

            if (!fieldName.equalsIgnoreCase(QueryUtils.KEY_FIELD_NAME) && !fieldName.equalsIgnoreCase(VAL_FIELD_NAME))
                res.put(fieldName, row.get(i));
        }
    ...
    private <V> V rowToEntity(List<?> row, FieldsQueryCursor<?> cursor) {
        ...
        try {
            res = (V)GridUnsafe.allocateInstance(type);
        }
        catch (InstantiationException e) {
            throw new IgniteException(
                "Unable to allocate instance of domain entity class " + type.getName(), e);
        }
        ...
        for (int i = 0; i < row.size(); i++) {
            ...
            if (fieldName == null ||
                fieldName.equalsIgnoreCase(KEY_FIELD_NAME) ||
                fieldName.equalsIgnoreCase(VAL_FIELD_NAME)
            )
                continue;

            fieldName = fieldName.toLowerCase();

            Object fieldVal = row.get(i);

            Field entityField = domainEntitiyFields.get(fieldName);

            if (entityField != null)
                GridUnsafe.putObjectField(res, GridUnsafe.objectFieldOffset(entityField),
                    fixExpectedType(fieldVal, entityField.getType()));
        }
