...
@EnableIgniteRepositories(
    value = {"org.apache.ignite.springdata20.compoundkey", "org.apache.ignite.springdata20.misc"},
...
public class IgniteClientApplicationConfiguration {
    ...
    public Ignite igniteServerNode() {
        return Ignition.start(igniteConfiguration(ApplicationConfiguration.IGNITE_INSTANCE_ONE, CompoundKeyApplicationConfiguration.CLI_CONN_PORT));
    }

    /** Ignite client instance bean with default name. */
    @Bean
    public IgniteClient igniteInstance() {
        return Ignition.startClient(new ClientConfiguration().setAddresses("127.0.0.1:" + CompoundKeyApplicationConfiguration.CLI_CONN_PORT)
            .setBinaryConfiguration(new BinaryConfiguration().setCompactFooter(false)));
    }

    /** Ignite client instance bean with not default name. */
    @Bean
    public IgniteClient igniteInstanceTWO() {
        Ignition.start(igniteConfiguration(ApplicationConfiguration.IGNITE_INSTANCE_TWO, 10801));
